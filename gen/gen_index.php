...
function dh_gen_index()
{
	...
	$topnews.=" <a href=\"".$DH_html_url."author.html\"> 详细 >> </a>";
...
function dh_gen_movie($DH_index_cat)
{
	...
	foreach($movietype as $movietype_index=>$movietype_name)
	{
		...
		if($movietype_index==1||$movietype_index==2)
		{
			...
			$liout .="\n".'<li class="normal" onclick="nTabs(this,'.$i.');">高清资源</li>';
		}
		else
		{
			//全部最新+精选最新
...
function dh_get_movie_type_block($sql,$i)
{
	...
	{									
		...
		while($row = mysql_fetch_array($recs))
		{
			...
			$lieach = "\n".'<li><div>'.$i.' <span class="line1 w100pre"><a href="'.$htmlpath.'" target="_blank" '.$titlea.'>'.$title.'</a> <span class="hotmeta1"> '.$update.'</span></span></div><div class="hotmeta1">['.$country.':'.$type.'] <span class="cred">'.$row['hot'].'</span></div></li>';
			$liout.= $lieach;
			$i++;
		}
		$liout.= "\n</ol>";	
...
function dh_get_movie_type_list($sql,$list_html,$type)
{
	...
	{			
		...
		{			
			...
			$DH_output_content_page = dh_replace_content(1,$row,$DH_output_content_page,false);
